...
btn.addEventListener('click', function () {
  ...
  for (const [i, item] of arr.entries()) {
    const newArr = item.trim().toLowerCase().split('_');
    let tempArry = [];
    for (let i = 1; newArr.length > i; i++) {
      tempArry += newArr[i].replace(newArr[i][0], newArr[i][0].toUpperCase());
    }
    console.log(tempArry);
    const finalArr = newArr[0] + tempArry;
    console.log(`${finalArr.padEnd(20)} ${'*'.repeat(i + 1)} `);
  }
});
...
const plane2 = 'Airbus A320 neo';
console.log(plane2.includes('Airbus A320 neo'));
...
console.log('Andy Wong'.split(' '));

const [firstName, lastName] = 'Andy Wong'.split(' ');
...
const fullName = 'alejandro perniche cavalazi';
capitalizeName(fullName);

// Padding
const message = 'Go to gate 23!';
console.log(message.padStart(25, '+').padEnd(35, '+'));
console.log('andy'.padStart(25, '+').padEnd(30, '+'));

const maskCreditCard = function (number) {
  const str = number + '';
  const lastFour = str.slice(-4);
  const masked = lastFour.padStart(number.length, '*');
  console.log(masked);
};

maskCreditCard('1234432112344321');
maskCreditCard('12344321');
// repeat
...
for (const item of flightArr) {
  const [type, from, to, time] = item.split(';');
  const output = `${type.startsWith('_Delayed') ? 'RED' : ' '}${type
    .replaceAll('_', ' ')
    .trim()
    .padStart(20)} from ${from.slice(0, 3).toUpperCase()} to ${to
    .slice(0, 3)
